...
#	- v1.1 = 13 Sept 2016
#            path fix for script to rewrite in uc
#            fix issue with headers > 50nt for RM (only my case with |ref|NW_XXX| taken care of though)
#	- v1.2 = 13 Sept 2016
#            option to not mask => just get the longest events in a table
\n";
...
my $usage = "\nUsage [v$version]: 
    ...
    OPTIONAL ARGUMENTS
     ...
     -n,--nomask  => (BOOL)   To only concat the files (no masking)
...
INPUT: foreach my $in (@{$listin}) {
	...
	print STDERR "         Concatenating $in/Del/Deletions.*/_ExtractAlign/_$id2.$fext in $id2_gaps \n" if ($v);
...
sub get_longest_del {
	...
	for (my $i=0; $i < $n; $i++) {
		...
		$fa_name =~ s/\.align\.fa$//;
		`cp $fa $fa_name`;
		#need to replace headers if too long; cases I encountered were with gi|..|ref|NW_XXX|, or gi|..|emb|CYUI_XXX|
		`sed -i 's/gi|[0-9A-Za-z\.]*|//' $fa_name`; 
		`sed -i 's/RevCom/RC/' $fa_name`;
	}
	return($data);
}
